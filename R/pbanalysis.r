...
#' Plot method
#' @param obj A pb object generated by the pb.fit function
#' @example R/examples/example_plot.R
#' @export
plot.pb <- function(obj){
  ...
  for(i in 1:dim(min)[2]){
    rc = colnames(min)[i]
    plotdf = rbind(plotdf,
                   cbind(rc,rep("Reference Group",dim(maj)[1]),
                         rownames(maj),
                         maj[,1],maj[,1],maj[,1],0))
    plotdf = rbind(plotdf,
  ...
  colnames(plotdf) <- c("race_comparison","quantity","outcome",
                        "val","low","upp","barsthere")


  plotdf = data.frame(plotdf)
  #for some reason these columns are not numeric, have to cast them
  plotdf$val = as.numeric(plotdf$val)
  plotdf$low = as.numeric(plotdf$low)
  plotdf$upp = as.numeric(plotdf$upp)
  options(warn=-1)
  plot <- ggplot2::ggplot(plotdf,ggplot2::aes(x=val,y=outcome,
                                              col = quantity,shape = quantity))+
    ggplot2::geom_point(ggplot2::aes(size=3)) +
    ggplot2::geom_errorbar(ggplot2::aes(xmin = low,xmax=upp,width = .1,alpha = barsthere))+
    ggplot2::facet_wrap(~race_comparison)+ ggplot2::guides(size = "none",
                                                           alpha = "none")
...
#' Summary method
#' @param obj A pb object generated by the pb.fit function
#' @example R/examples/example_summary.R
#' @export
summary.pb <- function(obj){
  ## return some summarized output
  out <- list(family = family,
              sample.sizes = obj$sample.sizes,
              reference.observed.mean = obj$ref.observed,
              minority.observed.mean = obj$observed.mean,
              counterfact.estimated.mean = obj$estimated.mean,
              overall.disparity = obj$overall.disp,
              unexplained.disparity = obj$unexplained.disp,
              unexp.disp.variance = obj$unexp.disp.variance)
