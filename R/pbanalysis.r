...
plot.pb <- function(obj){
  ...
  if(obj$family == "gaussian"){
    ...
    colnames(plotdf) <- c("race_comparison","quantity","outcome","val")
    plotdf = data.frame(plotdf)

    ggplot(plotdf,aes(x=val,y=outcome,shape = quantity))+geom_point(aes(size=3)) +
      facet_wrap(~race_comparison)+ guides(size = "none")
  }
}
#' Summary method
#' @param obj A pb object generated by the pb.fit function
